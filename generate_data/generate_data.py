...
def get_mask_aug(img_path):
    ...
    if result.masks is None:
        return None, None
    cv2.imwrite("./original_image.jpg", result.orig_img)
    ...
    for i, obj_index in enumerate(obj_indices[0].cpu().numpy()):
        ...
        print(obj_mask_resized[0][0], obj_mask_resized[400][500])
        cropped_img = cv2.bitwise_and(img, img, mask=obj_mask_resized)
        image = Image.open(img_path)
        image, x1, y1, x2, y2 = add_defects(image, obj_mask_resized)
...
def main():
    ...
    for i in range(200, 300):
        ...
        img_filename = f"dataset/{folder}/images/round_no_metal_{i}.jpg"
        isulator.save(img_filename)
        print(img_filename)
        # Сохранение текстового файла
        txt_filename = f"dataset/{folder}/labels/round_no_metal_{i}.txt"
